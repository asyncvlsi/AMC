...
class multi_bank(design.design):
    ...
    def compute_bus_sizes(self):
        ...
        if self.orien == "V":
            self.data_bus_width= self.bank.width + self.bnk_to_bus_gap+ max(self.sp_mrg_ctrl.height, 2*self.w_size*self.m_pitch("m1"))
            if ((2*self.w_size+2)*self.m_pitch("m1")>self.sp_mrg_ctrl.height-self.v_bus_width) :
                self.data_bus_width= self.data_bus_width+ (2*self.w_size+2)*self.m_pitch("m1")-self.sp_mrg_ctrl.height+self.v_bus_width
    ...
    def compute_two_bank_offsets(self):
        ...
        self.bank_ack_mrg_off=self.bank.get_pin("ack_merge").ll().y + self.m_pitch("m1")
    ...
    def add_two_banks(self):
        ...
        if self.orien == "H":
            ...
            self.width= self.bank_inst[1].ur().x + self.m2_width
        ...
        if self.orien == "V":
            ...
            if (self.sp_mrg_ctrl.height >= self.v_bus_width):
                self.width= self.bank_inst[1].ur().x + (self.sp_mrg_ctrl.height-self.v_bus_width) + \
                            self.m2_width
            else:
                self.width= self.bank_inst[1].ur().x + self.m2_width
            ...
            if ((2*self.w_size+1)*self.m_pitch("m1")>(self.sp_mrg_ctrl.height-self.v_bus_width)):
                self.width = self.bank_inst[1].ur().x + self.m2_width + \
                             (2*self.w_size+1)*self.m_pitch("m1")-(self.sp_mrg_ctrl.height-self.v_bus_width)
        ...
        self.height= max(self.bank_inst[1].ur().y, self.sp_mrg_ctrl_inst.ur().y)
    ...
    def add_four_banks(self):
        ...
        if self.orien == "V":
            self.height= max(self.bank_inst[3].ur().y, self.sp_mrg_ctrl_inst.ur().y)
    ...
    def add_single_bank_pins(self):
        ...
        for i in range(self.w_size):
            if self.num_subanks > 1:
                pin= self.bank_inst.get_pin("din[0][{0}]".format(i)).ll()
                offset= vector(pin.x, pin.y-(i+4)*self.m_pitch("m1"))
                width= self.bank_inst.get_pin("din[{0}][{1}]".format(self.num_subanks-1, i)).ll().x-\
                       offset.x
                ...
                for j in range(self.num_subanks):
                    shift= self.bank_inst.get_pin("din[{0}][{1}]".format(j, i)).ll().x-offset.x
                    self.add_rect(layer= "metal2", 
                                  offset= (offset.x + shift, offset.y), 
                                  width= self.m2_width, 
                                  height= (i+4)*self.m_pitch("m1"))
                    self.add_via(self.m1_stack,(offset.x + shift, offset.y))
            self.add_layout_pin(text="din[{0}]".format(i),
                                layer= self.m2_pin_layer,
                                offset= self.bank_inst.get_pin("din[0][{0}]".format(i)).ll(),
                                width= self.m2_width,
                                height= self.m2_width)
            
        for i in range(self.w_size):
            if self.num_subanks > 1:
                pin=self.bank_inst.get_pin("dout[0][{0}]".format(i)).ll()
                offset= vector(pin.x, pin.y-(i+4+self.w_size)*self.m_pitch("m1"))
                width= self.bank_inst.get_pin("dout[{0}][{1}]".format(self.num_subanks-1, i)).ll().x-\
                       offset.x
                self.add_rect(layer= "metal1", 
                              offset= offset, 
                              width= width, 
                              height= self.m1_width)
                for j in range(self.num_subanks):
                    shift= self.bank_inst.get_pin("dout[{0}][{1}]".format(j, i)).ll().x-offset.x
                    self.add_rect(layer= "metal2", 
                                  offset= (offset.x + shift, offset.y), 
                                  width= self.m2_width, 
            ...
            self.add_layout_pin(text="dout[{0}]".format(i),
                                layer= self.m2_pin_layer,
                                offset= self.bank_inst.get_pin("dout[0][{0}]".format(i)).ll(),
                                width= self.m2_width,
                                height= self.m2_width)
        ...
        for i in range(2):
            for j in range(self.num_subanks):
                offset1= vector(self.bank_inst.get_pins(power_pin[i])[self.num_subanks-j].uc().x,
                                self.bank_inst.get_pins(power_pin[i])[self.num_subanks-j].ll().y)
                offset2= vector(self.bank_inst.get_pins(power_pin[i])[self.num_subanks-j-1].uc().x,
                                self.bank_inst.get_pins(power_pin[i])[self.num_subanks-j-1].ll().y)
                self.add_wire(self.m1_stack,
                              [offset1, (offset1.x, offset1.y-(1.5+i)*self.m_pitch("m1")),
                              (offset2.x, offset2.y-(1.5+i)*self.m_pitch("m1")), offset2])

            power_pin_off = self.bank_inst.get_pins(power_pin[i])[self.num_subanks]
            self.add_layout_pin(text=power_pin[i],
                                layer= self.m1_pin_layer,
                                offset= (power_pin_off.uc().x-0.5*self.m1_width,
                                         power_pin_off.ll().y-(1.5+i)*self.m_pitch("m1")),
                                width= self.m1_width,
                                height= self.m1_width)
    ...
    def route_banks(self):
        ...
        if (self.num_banks == 2 or self.orien == "H"):
            for k in range(self.num_banks):
                for i in range(self.num_subanks):
                  for j in range(self.w_size):
                      din_off= vector(self.bank_inst[k].get_pin("din[{0}][{1}]".format(i,j)).ll().x, 
                                      self.din1_bus_off.y+ j*self.m_pitch("m1") + 0.5*self.m1_width)
                      din_height=  self.bank_inst[k].get_pin("din[{0}][{1}]".format(i,j)).ll().y -\
                      ...
                      dout_off= vector(self.bank_inst[k].get_pin("dout[{0}][{1}]".format(i,j)).ll().x, 
                                       self.dout1_bus_off.y+ j*self.m_pitch("m1") + 0.5*self.m1_width)
                      dout_height=  self.bank_inst[k].get_pin("dout[{0}][{1}]".format(i,j)).ll().y -\
        ...
        for k in range(self.num_banks):
            for i in range(self.bank_addr_size):
                ...
                addr_off= vector(self.addr_bus1_off.x+ i*self.m_pitch("m1"), 
                ...
                self.add_rect(layer=layer, 
                              ...
                              width=addr_width, 
                              height=height)
                self.add_via(stack, (addr_off.x-0.5*self.m2_width, addr_off.y))
        ...
        for k in range(self.num_banks):
            ...
            self.add_rect(layer=layer, 
                          ...
                          height=height)
            self.add_via(stack, bank_sel_off)
        ...
        for k in range(self.num_banks):
            ...
            for i in range(len(control_pin_list)):
                ...
                self.add_rect(layer=layer, 
                              ...
                              width=ctrl_width, 
                              height=height)
                self.add_via(stack, (ctrl_off.x, ctrl_off.y))             
        ...
        if not self.two_level_bank:
            ...
            self.add_via(self.m1_stack, (sel_pos1.x-0.5*self.m2_width, sel_pos1.y-0.5*self.m1_width))
            self.add_via(self.m1_stack, (sel_pos2.x-0.5*self.m2_width, sel_pos2.y-0.5*self.m1_width))
        ...
        if (self.num_banks == 2 or self.orien == "H"):
            for k in range(self.num_banks):
                for vdd_pin in self.bank_inst[k].get_pins("vdd"):
                    vdd_off= vector(vdd_pin.ll().x, self.pow_rail_1_off.y + 0.5*self.m1_width)
                    vdd_height=  vdd_pin.ll().y - self.pow_rail_1_off.y
                    self.add_rect(layer="metal2", 
                                  offset=vdd_off, 
                ...
                for gnd_pin in self.bank_inst[k].get_pins("gnd"):
                    gnd_off= vector(gnd_pin.ll().x, 
                                    self.pow_rail_1_off.y + self.pow_rail_pitch + 0.5*self.m1_width)
                    gnd_height=  gnd_pin.ll().y - self.pow_rail_1_off.y - self.pow_rail_pitch
                    self.add_rect(layer="metal2", 
                                  offset=gnd_off, 
        ...
        if (self.num_banks == 4 and  self.orien == "V"):
            ...
            for k in range(self.num_banks):
                for vdd_pin in self.bank_inst[k].get_pins("vdd"):
                    vdd_off= vector(vdd_pin.ll().x, self.power_rail_off[k/2] + 0.5*self.m1_width)
                    vdd_height=  vdd_pin.ll().y - self.power_rail_off[k/2]
                    self.add_rect(layer="metal2", 
                                  offset=vdd_off, 
                                  width=self.bank.vdd_rail_width, 
                                  height=vdd_height)
                ...
                for gnd_pin in self.bank_inst[k].get_pins("gnd"):
                    gnd_off= vector(gnd_pin.ll().x, self.power_rail_off[k/2] + \
                                    self.pow_rail_pitch + 0.5*self.m1_width)
                    gnd_height=  gnd_pin.ll().y - self.power_rail_off[k/2] - self.pow_rail_pitch
                    self.add_rect(layer="metal2", 
                                  offset=gnd_off, 
                                  width=self.bank.gnd_rail_width, 
                                  height=gnd_height)
        ...
        if (self.num_banks == 4 and  self.orien == "V"):
            self.add_via(self.m1_stack, (self.vdd_off.x, self.pow_rail_2_off.y+0.5*self.m1_width))
            self.add_via(self.m1_stack, 
                        (self.gnd_off.x,self.pow_rail_2_off.y+self.pow_rail_pitch+0.5*self.m1_width))
...
#/////////////////////////////////////////////////////////////////////////////#
    ...
    def add_split_merge_cells(self):
        ...
        if (self.num_banks == 2 and self.orien=="H"):
            y_off= -(max(self.addr_size,self.control_size)+2)*self.m_pitch("m2")-\
                     self.addr_split_ary.height
        else:
            y_off= -(max(max(self.addr_size,self.control_size), self.w_size)+2)*self.m_pitch("m2")-\
                     self.addr_split_ary.height
        ...
        if self.num_banks == 2:
            ...
            self.height= max(self.bank_inst[1].ur().y, self.sp_mrg_ctrl_inst.ur().y) -\
                             self.dsplit_ary_inst.ll().y + 8*self.m_pitch("m1")

        if self.num_banks == 4:
            self.width= self.bank_inst[1].ur().x + (2*self.w_size-1)*self.m_pitch("m1")
            # 8 m1 pitch fo split and merge control signals
            self.height= max(self.bank_inst[1].ur().y, self.sp_mrg_ctrl_inst.ur().y) -\
                             self.dsplit_ary_inst.ll().y + 8*self.m_pitch("m1")
        ...
        if self.orien == "V":
            ...
            if self.num_banks == 2:
                y_off2= self.bank_pos_1.y + self.bank.height +\
                        self.dmerge_ary.height + (self.w_size+1)*self.m_pitch("m1")
            if self.num_banks == 4:
            ...
            for i in range(self.w_size):
                self.add_layout_pin(text="dout[{0}]".format(i), 
                                    layer=self.m2_pin_layer, 
                                    offset= (self.dmerge_ary_inst.get_pin("Q[{0}]".format(i)).ll().x,
                                             self.dmerge_ary_inst.ul().y-self.m2_width),
                                    width=self.m2_width, 
                                    height=self.m2_width)

            self.width= self.bank_inst[1].ur().x + (self.sp_mrg_ctrl.height-self.v_bus_width)+\
                        (self.w_size+7)*self.m_pitch("m1")
            self.height= max(self.sp_mrg_ctrl_inst.ur().y, self.dmerge_ary_inst.ur().y) -\
                         self.dsplit_ary_inst.ll().y + 8*self.m_pitch("m1")
            
            
            
            if ((2*self.w_size+1)*self.m_pitch("m1")>(self.sp_mrg_ctrl.height-self.v_bus_width)):
                self.width = self.bank_inst[1].ur().x + (self.w_size+7)*self.m_pitch("m1") + \
        ...
        x_off= max(self.sp_mrg_ctrl_inst.lr().x + self.ctrl_split_gap, 
                   self.addr_split_ary_inst.lr().x + self.ctrl_split_gap)
        ...
        x_off= self.ctrl_split_ary_inst.lr().x + self.ctrl_split_gap
        ...
        x_off= self.ack_mrg_inst.lr().x + self.ctrl_split_gap
        ...
        x_off= self.rack_mrg_inst.lr().x + self.ctrl_split_gap
    ...
    def route_data_split_merge(self):
        ...
        if (self.num_banks == 2 and self.orien == "H"):
            ...
            for i in range(self.w_size):
                ...
                self.add_wire(self.m1_stack, 
                              ...
                              (data_in_pos_x+i*self.m_pitch("m1"),data_in_pos_y),
                              (data_in_pos_x+i*self.m_pitch("m1"),self.din_split_bus_off[i][1]),
                              (self.din_split_bus_off[i][0],
                               self.din_split_bus_off[i][1]+0.5*self.m1_width)]) 

            self.add_via(self.m1_stack, 
                         (self.din1_bus_pos["din_split[0]"][0]+self.data_bus_width-0.5*self.m2_width, 
                          self.din1_bus_pos["din_split[0]"][1]-0.5*self.m1_width))

        # connecting data_in_split_array to data_in_split_bus
        for i in range(self.w_size):
            data_in_pos= self.dsplit_ary_inst.get_pin("Q[{0}]".format(i)).uc()
            x_off= self.bank_inst[self.num_banks-1].get_pin("din[{0}][{1}]".format(self.num_subanks-1,i)).ll().x
            y_off= self.din_split_bus_off[i][1]
            self.add_via(self.m1_stack, (x_off, y_off))
            ...
            if self.num_banks == 4:
                for i in range(self.w_size):
                    ...
                    self.add_wire(self.m1_stack, 
                                  ...
                                  (data_out_pos_x-i*self.m_pitch("m1"), data_out_pos_y),
                                  (data_out_pos_x-i*self.m_pitch("m1"), self.dout_mrg_bus_off[i][1]),
                                  (self.dout_mrg_bus_off[i][0], 
                                   self.dout_mrg_bus_off[i][1]+0.5*self.m1_width)]) 

                self.add_via(self.m1_stack, 
                            (self.dout1_bus_pos["dout_merge[0]"][0]-0.5*self.m2_width, 
                             self.dout1_bus_pos["dout_merge[0]"][1]-0.5*self.m1_width))

                self.add_via(self.m1_stack, (self.dout_mrg_bus_off[0][0]-0.5*self.m1_width,
                                             self.dout_mrg_bus_off[0][1]))
        ...
        if self.orien == "V":
            ...
            if (self.num_banks == 2 and self.orien == "V") :
                self.add_via(self.m1_stack, (self.dout1_bus_pos["dout_merge[0]"][0]+\
                             self.data_bus_width-0.5*self.m2_width, 
                             self.dout1_bus_pos["dout_merge[0]"][1]-0.5*self.m1_width))
            if (self.num_banks == 4 and self.orien == "V"):
                self.add_via(self.m1_stack,(self.dout2_bus_pos["dout_merge[0]"][0]+\
                             self.data_bus_width-0.5*self.m2_width, 
                             self.dout2_bus_pos["dout_merge[0]"][1]-0.5*self.m1_width))


        # connecting data_out_merge_array to data_out_merge_bus
        for i in range(self.w_size):
            if self.orien=="V":
                data_out_pos= self.dmerge_ary_inst.get_pin("D[{0}]".format(i)).lr()
            if self.orien=="H":
                data_out_pos= self.dmerge_ary_inst.get_pin("D[{0}]".format(self.w_size-1-i)).ul()
            x_off= self.bank_inst[0].get_pin("dout[{0}][{1}]".format(self.num_subanks-1,i)).ll().x
            y_off= self.dout_mrg_bus_off[i][1]
            self.add_path("metal2", [(x_off+0.5*contact.m1m2.width, y_off), data_out_pos], 
    ...
    def route_addr_ctrl_split_merge(self):
        ...
        for i in range(self.addr_size):
            ...
            self.add_via(self.m2_stack,(addr_bus_pos[0]-0.5*self.m3_width,
                                        addr_split_y_off-0.5*self.m3_width))
    ...
    def route_split_cells_powers_and_selects(self):
        ...
        for i in range(4):
            ...
            if self.orien == "V":
                x_off= self.sp_mrg_ctrl_inst.ll().x

            y_off=self.addr_split_ary_inst.ll().y
        ...
        for i in range(3):
            for mod in (mod_list1 + mod_list2):
                ...
                self.add_wire(self.m1_stack,[power_pos.lc(), 
                              (power_pos.ll().x-(1+i)*self.m_pitch("m1"), power_pos.lc().y), 
                              (power_pos.ll().x-(1+i)*self.m_pitch("m1"), y_off-(i+1)*self.m_pitch("m1"))])
                self.add_via(self.m1_stack,(power_pos.ll().x-(1+i)*self.m_pitch("m1")-\
        ...
        for mod in mod_list1:
            ...
            self.add_wire(self.m1_stack, [select_pos.lc(),
                          (select_pos.ll().x-4*self.m_pitch("m1"), select_pos.lc().y), 
                          (select_pos.ll().x-4*self.m_pitch("m1"), y_off-4*self.m_pitch("m1"))])
            self.add_via(self.m1_stack, (select_pos.ll().x-4*self.m_pitch("m1")-0.5*self.m2_width,
                                         y_off-4*self.m_pitch("m1"))) 

        for mod in mod_list2:
            select_pos= mod.get_pin("M")
            self.add_wire(self.m1_stack, [select_pos.lc(),(select_pos.ll().x-4*self.m_pitch("m1"), 
                          select_pos.lc().y), 
                          (select_pos.ll().x-4*self.m_pitch("m1"),y_off-4*self.m_pitch("m1"))])
            self.add_via(self.m1_stack, (select_pos.ll().x-4*self.m_pitch("m1")-0.5*self.m2_width,
                                         y_off-4*self.m_pitch("m1"))) 
        ...
        for i in range(2):
            ...
            self.add_wire(self.m1_stack, [select_pos.lc(), 
                          (select_pos.ll().x-(i+5)*self.m_pitch("m1"), select_pos.lc().y), 
                          (select_pos.ll().x-(i+5)*self.m_pitch("m1"), 
                          self.ctrl_split_ary_inst.ll().y-6*self.m_pitch("m1"))])
            self.add_layout_pin(text=select_split_pin_name[i],
                                layer= self.m2_pin_layer,
                                offset=(select_pos.ll().x-(i+5)*self.m_pitch("m1")-0.5*self.m2_width,
                                        self.ctrl_split_ary_inst.ll().y-6*self.m_pitch("m1")),
                                width= self.m2_width,
                                height= self.m2_width)
        ...
        for mod in mod_list2:
            ...
            self.add_wire(self.m1_stack, [select_pos1.lc(), 
                          (select_pos1.ll().x-5*self.m_pitch("m1"), select_pos1.lc().y), 
                          (select_pos1.ll().x-5*self.m_pitch("m1"), 
                           self.ctrl_split_ary_inst.ll().y-6*self.m_pitch("m1"))])
            self.add_layout_pin(text=select_merge_pin_name[mod_list2.index(mod)],
                                layer= self.m2_pin_layer,
                                offset=(select_pos1.ll().x-5*self.m_pitch("m1")-0.5*self.m2_width,
                                        self.ctrl_split_ary_inst.ll().y-6*self.m_pitch("m1")),
                                width= self.m2_width,
                                height= self.m2_width)
        ...
        for mod in mod_list:           
            ...
            self.add_wire(self.m1_stack, [select_pos2.lc(),
                          (select_pos2.ll().x-6*self.m_pitch("m1"), select_pos2.lc().y), 
                          (select_pos2.ll().x-6*self.m_pitch("m1"), 
        ...
        for i in range(4):
            ...
            if self.orien == "H":
                y_off= self.dmerge_ary_inst.ll().y
                self.add_wire(self.m1_stack, [select_pos.lc(), 
                              (self.dmerge_ary_inst.ur().x+(1+i)*self.m_pitch("m1"), 
                                select_pos.lc().y), 
                              (self.dmerge_ary_inst.ur().x+(1+i)*self.m_pitch("m1"), 
                                y_off-(1+i)*self.m_pitch("m1"))])
                self.add_via(self.m1_stack, 
                            (select_pos.ll().x+self.dmerge_ary.width+(1+i)*self.m_pitch("m1")-\
                            0.5*self.m2_width,y_off-(1+i)*self.m_pitch("m1"))) 
            
            if self.orien == "V":
                y_off= self.dsplit_ary_inst.ll().y
                self.add_wire(self.m1_stack, [select_pos.lc(), 
                              (self.dmerge_ary_inst.ur().x+(1+i+self.w_size)*self.m_pitch("m1"), 
                               select_pos.lc().y), 
                              (self.dmerge_ary_inst.ur().x+(1+i+self.w_size)*self.m_pitch("m1"), 
                               y_off-(1+i)*self.m_pitch("m1")),
                              (select_pos.ll().x, y_off-(1+i)*self.m_pitch("m1")+0.5*self.m1_width)])
        ...
        for i in range(2):
            ...
            self.add_wire(self.m1_stack, [(spl_mrg_power.ll().x, spl_mrg_power.lc().y),
                         (spl_mrg_power.ll().x-(i+8)*self.m_pitch("m1"), spl_mrg_power.lc().y),
                         (spl_mrg_power.ll().x, self.reset_off.y-(2-i)*self.m_pitch("m1")),
                         (bank_power.uc().x, self.reset_off.y-(2-i)*self.m_pitch("m1")),
                         (bank_power.uc().x, self.reset_off.y)])
        ...
        sel_pos3=vector(sel_pos2.x, self.addr_split_ary_inst.ll().y-4*self.m_pitch("m1"))
        ...
        self.add_wire(self.m1_stack, [(spl_mrg_reset.ll().x, spl_mrg_reset.lc().y),
                     (spl_mrg_reset.ll().x-3*self.m_pitch("m1"), spl_mrg_reset.lc().y),
                     (spl_mrg_reset.ll().x-3*self.m_pitch("m1"),self.reset_off.y-3*self.m_pitch("m1")),
        ...
        for i in range(2):
            ...
            yoff= self.ctrl_split_ary_inst.ll().y-(5+i)*self.m_pitch("m1")
            self.add_wire(self.m1_stack,
                          [en_addr.lc(), (en_addr.ll().x-(i+5)*self.m_pitch("m1"), en_addr.lc().y), 
                          (en_addr.ll().x-(i+5)*self.m_pitch("m1"), yoff), 
                          (en_data.ll().x-(i+5)*self.m_pitch("m1"), yoff),
                          (en_data.ll().x-(i+5)*self.m_pitch("m1"), en_data.lc().y), en_data.lc()])

            self.add_wire(self.m1_stack,
                          [en_addr.lc(), (en_addr.ll().x-(i+5)*self.m_pitch("m1"), en_addr.lc().y), 
                          (en_addr.ll().x-(i+5)*self.m_pitch("m1"), yoff), 
                          (en_ctrl.ll().x-(i+5)*self.m_pitch("m1"), yoff),
                          (en_ctrl.ll().x-(i+5)*self.m_pitch("m1"), en_ctrl.lc().y), en_ctrl.lc()])
        ...
        yoff= self.ctrl_split_ary_inst.ll().y-7*self.m_pitch("m1")
        if self.orien == "H":
            self.add_wire(self.m1_stack,
                          [en_ctrl.lc(), (en_ctrl.ll().x-5*self.m_pitch("m1"), en_ctrl.lc().y), 
                          (en_ctrl.ll().x-5*self.m_pitch("m1"), yoff), 
                          (self.dmerge_ary_inst.ur().x+5*self.m_pitch("m1"), yoff),
                          (self.dmerge_ary_inst.ur().x+5*self.m_pitch("m1"), en_data.lc().y), 
                           en_data.lc()])
        ...
        if self.orien == "V":
            self.add_wire(self.m1_stack,
                          [en_ctrl.lc(), (en_ctrl.ll().x-5*self.m_pitch("m1"), en_ctrl.lc().y), 
                          (en_ctrl.ll().x-5*self.m_pitch("m1"), yoff), 
                          (self.dmerge_ary_inst.ur().x+(5+self.w_size)*self.m_pitch("m1"), yoff),
                          (self.dmerge_ary_inst.ur().x+(5+self.w_size)*self.m_pitch("m1"), 
        ...
        yoff= self.ctrl_split_ary_inst.ll().y-8*self.m_pitch("m1")
        if self.orien == "H":
            self.add_wire(self.m1_stack,
                          [en_ctrl.lc(), (en_ctrl.ll().x-6*self.m_pitch("m1"), en_ctrl.lc().y), 
                          (en_ctrl.ll().x-6*self.m_pitch("m1"), yoff), 
                          (self.dmerge_ary_inst.ur().x+6*self.m_pitch("m1"), yoff),
                          (self.dmerge_ary_inst.ur().x+6*self.m_pitch("m1"), en_data.lc().y), 
                           en_data.lc()])
        ...
        if self.orien == "V":
            self.add_wire(self.m1_stack,
                          [en_ctrl.lc(), (en_ctrl.ll().x-6*self.m_pitch("m1"), en_ctrl.lc().y), 
                          (en_ctrl.ll().x-6*self.m_pitch("m1"), yoff), 
                          (self.dmerge_ary_inst.ur().x+(6+self.w_size)*self.m_pitch("m1"), yoff),
                          (self.dmerge_ary_inst.ur().x+(6+self.w_size)*self.m_pitch("m1"), 
                           en_data.lc().y), en_data.lc()])
