...
class column_mux(design.design):
    ...
    def add_ptx(self):
        ...
        self.nmos1_position = vector(self.nmos.height+self.implant_enclose_poly, 
        ...
        self.nmos2_position = vector(self.bitcell.width-self.implant_enclose_poly, 
    ...
    def connect_poly(self):
        ...
        offset = (self.nmos1.get_pin("G").lc().x+ x_off, self.nmos1.get_pin("G").ll().y)
    ...
    def add_bitline_pins(self):
        ...
        bl_pos = vector(self.bitcell.get_pin("bl").ll().x, 0)
        br_pos = vector(self.bitcell.get_pin("br").ll().x, 0)
    ...
    def connect_bitlines(self):
        ...
        self.add_via_center(self.m1_stack, nmos1_d_pin, rotate=90)
        self.add_via_center(self.m1_stack, nmos2_s_pin, rotate=90)
        self.add_via_center(self.m1_stack, nmos2_d_pin, rotate=90)
        
        self.add_m1_minarea(nmos1_s_pin)
        self.add_m1_minarea(nmos1_d_pin)
        self.add_m1_minarea(nmos2_s_pin)
        self.add_m1_minarea(nmos2_d_pin)
        
        self.add_path("metal2",[(bl_pin.uc().x, -contact.well.height-self.pin_height),nmos1_s_pin], 
                      width=contact.m1m2.width)
        self.add_path("metal2", [(bl_pin.uc().x,self.top), nmos1_d_pin], 
                      width=contact.m1m2.width)
        self.add_path("metal2",[(br_pin.uc().x, -contact.well.height-self.pin_height),nmos2_s_pin], 
                      width=contact.m1m2.width)
        self.add_path("metal2",[(br_pin.uc().x,self.top), nmos2_d_pin], 
                      width=contact.m1m2.width)

    def add_gnd_rail(self):
        """ Add the gnd rails that span the whole cell"""

        self.gnd_position = vector(0, self.nmos1.ll().y-self.m1_width)
    ...
    def add_well_contact(self):
        ...
        well_contact_offset = vector(self.well_enclose_active + contact.well.width, 
                                     self.nmos1.ll().y - self.m1_width - contact.well.height)
        self.add_contact(layers=("active", "contact", "metal1"), 
                         offset=well_contact_offset, implant_type="p", well_type="p")
        
        active_width= self.active_minarea/self.active_width
        active_height = self.active_width
        active_offse=(self.well_enclose_active,  well_contact_offset.y+ \
        ...
        self.add_rect(layer="pimplant", 
                      ...
                      width = self.width,
                      height= self.nmos1.ll().y+contact.well.height+self.pin_height)

        self.add_rect(layer="nimplant", 
                      offset=(0, self.nmos1.ll().y), 
                      width =self.width ,
                      height= self.top-self.nmos1.ll().y)
