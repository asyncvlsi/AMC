...
class wire(path):
    ...
    def create_vias(self):
        """ Add a 1x1 via and corner square at every corner of the path."""
        ...
        for (a, offset, c) in list(threewise):
            ...
            if a[1] == c[1]:
                continue
            via_offset = [offset[0] + 0.5*c_height, offset[1] - 0.5*c_width]
            self.obj.add_via(layers=self.layer_stack, offset=via_offset, rotate=90)
    ...
    def create_rectangles(self):
        ...
        for index in range(len(pl) - 1):
            if pl[index][0] != pl[index + 1][0]:
                ...
                temp_offset = [pl[index][0], pl[index][1] - 0.5*self.horiz_layer_width]
                if line_length < 0:
                    temp_offset = [temp_offset[0] + line_length, temp_offset[1]]
                ...
                temp_offset = [pl[index][0] - 0.5 * self.vert_layer_width, pl[index][1]]
                if line_length < 0:
                    temp_offset = [temp_offset[0], temp_offset[1] + line_length]
    ...
    def assert_node(self, A, B):
        """ Check if the node movements are not big enough for the technology sizes."""
        ...
        if X_diff == 0 and Y_diff == 0:
            ...
            if X_diff == 0:
                assert Y_diff >= minY, "node" +  str(A) + " and node" + str(B) + \
                        " are too close in Y. Minmum is " + str(minX)
            if Y_diff == 0:
                assert X_diff >= minX, "node" +  str(A) + " and node" + str(B) + \
