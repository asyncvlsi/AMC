...
def create_rectilinear_route(my_list):
    """ Add intermediate nodes if it isn't rectilinear. Skip repeated nodes. 
        Also, convert to vector if the aren't."""
...
class path():
    """ Object metal path; given the layer type. Add a path of minimium metal width between 
    ...
    def create_rectangles(self):
        ...
        for index in range(len(pl) - 1):
            ...
            if pl[index][0] != pl[index + 1][0]:
                ...
                offset = [pl[index][0], pl[index][1] - 0.5 * self.layer_width]
                if line_length < 0:
            ...
            # if we have y motion
                ...
                offset = [pl[index][0] - 0.5 * self.layer_width, pl[index][1]]
                if line_length < 0:
                    offset = [offset[0], offset[1] + line_length]
                self.add_line(layer_name=self.layer_name,
                              length=abs(line_length),
                              offset=offset,
    ...
    def add_line(self, layer_name, length, offset, orientation, layer_width):
        ...
        if orientation == "horizontal":
            width = length
            height = layer_width
        self.obj.add_rect(layer=layer_name, offset=offset, width=width, height=height)
