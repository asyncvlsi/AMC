...
class ptx(design.design):
    ...
    def __init__(self, width=drc["minwidth_tx"], mults=1, tx_type="nmos", connect_active=False, 
                       connect_poly=False, num_contacts=None, min_area=True):
    ...
    def setup_layout_constants(self):
        ...
        if ((self.active_width*self.tx_width) < drc["minarea_active"] and self.min_area):
            end_to_poly = (drc["minarea_active"] / (2*self.tx_width)) - (self.poly_width/2)
        ...
        if info["has_{}well".format(self.well_type)]:
            ...
            self.cell_well_height = max(self.tx_width + 2*self.well_enclose_active, self.well_width)
            self.height = self.cell_well_height
        ...
        self.contact_offset = self.active_offset + vector(self.active_enclose_contact + \
    ...
    def connect_fingered_active(self, drain_positions, source_positions):
        ...
        pin_offset = vector(0, 0.5*max(self.active_height, self.active_contact.height,
                                       self.active_contact.width) + self.m1_space + 0.5*self.m1_width)
        ...
        if len(source_positions)>1: 
            ...
            source_pin_offset=source_positions[0]+source_offset-end_offset+vector(0.5*self.m1_width,0)
            self.add_layout_pin_center_rect(text="S",
                                            layer=self.m1_pin_layer,
        ...
        if len(drain_positions)>1:
            ...
            drain_pin_offset=drain_positions[0]+drain_offset-end_offset+vector(0.5*self.m1_width,0)
            self.add_layout_pin_center_rect(text="D",
                                            layer=self.m1_pin_layer,
    ...
    def add_well_implant(self):
        ...
        if info["has_{}well".format(self.well_type)]:
            ...
            if self.connect_active:
                ...
                if self.tx_width < self.active_contact.height:
                    shift = (self.active_contact.height-self.tx_width)/2
                else: 
                    shift = 0
                m1_extend = self.m1_space+self.m1_width-self.well_enclose_active
                if m1_extend > 0 : 
                    self.cell_well_height = self.cell_well_height + m1_extend 
                    if shift > 0:
                        self.cell_well_height = self.cell_well_height - (shift - m1_extend)
                ...
                # for nmos tx, m1 connection for S contact is below active
                    ...
                    self.well_offset=self.well_offset- vector(0, m1_extend+shift)
            
            self.cell_well_height = max(self.cell_well_height, self.well_width)
            self.height = self.cell_well_height
    ...
    def create_spice(self):
        ...
        self.spice_device="M{{0}} {{1}} {0} m={1} w={2}u l={3}u pd={4}u ps={4}u as={5}p ad={5}p".format(
